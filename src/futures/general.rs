...
#[derive(Clone)]
pub struct FuturesGeneralCoinM {
    pub client: Client,
}
...
impl FuturesGeneralCoinM {
    // Test connectivity
    pub fn ping(&self) -> Result<String> {
        self.client.get(API::FuturesCoin(FuturesCoin::Ping), None)?;
        Ok("pong".into())
    }

    // Check server time
    pub fn get_server_time(&self) -> Result<ServerTime> {
        self.client.get(API::FuturesCoin(FuturesCoin::Time), None)
    }

    // Obtain exchange information
    // - Current exchange trading rules and symbol information
    pub fn exchange_info(&self) -> Result<ExchangeInformation> {
        self.client.get(API::FuturesCoin(FuturesCoin::ExchangeInfo), None)
    }

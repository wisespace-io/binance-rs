...
use crate::model::{
    AssetDetail, CoinInfo, DepositAddress, SpotFuturesTransferType,
    TransactionId,
};
...
impl Savings {
        ...
        S: Into<String>,
    {
        ...
        self.client.get_signed(
            API::Savings(Sapi::DepositAddress),
            Some(request),
        )
    }
        ...
        S: Into<String>,
    {
        ...
        parameters
            .insert("type".into(), (transfer_type as u8).to_string());
        let request =
            build_signed_request(parameters, self.recv_window)?;
        self.client.post_signed(
            API::Savings(Sapi::SpotFuturesTransfer),
            request,
        )
    }
}

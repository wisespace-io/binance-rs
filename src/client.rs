...
impl Client {
    ...
    fn sign_request(
        ...
        if let Some(request) = request {
            let mut signed_key = Hmac::<Sha256>::new_from_slice(
                self.secret_key.as_bytes(),
            )
            .unwrap();
            signed_key.update(request.as_bytes());
            let signature =
                hex_encode(signed_key.finalize().into_bytes());
            let request_body: String =
                format!("{}&signature={}", request, signature);
            format!(
                "{}{}?{}",
                self.host,
            ...
            let signed_key = Hmac::<Sha256>::new_from_slice(
                self.secret_key.as_bytes(),
            )
            .unwrap();
            let signature =
    ...
    fn build_headers(&self, content_type: bool) -> Result<HeaderMap> {
        ...
        custom_headers
            .insert(USER_AGENT, HeaderValue::from_static("binance-rs"));
        if content_type {
